...
bool inline_left_right(double d, int lane)
{
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // define some flags for decision making
                    bool slow_down = false;
                    bool keep_fast = false;
                    bool left_lane_empty = true;
                    bool right_lane_empty = true;
                    bool change_lane = false;

                    // check sensor fusion data to identify other cars' position and velocity
                    for (int i = 0; i < sensor_fusion.size(); i++)
                    {
                        // each car has: [ id, x, y, vx, vy, s, d]
                        ...
                        check_car_s += ((double)prev_size * .02 * check_speed); // a position of a car

                        // where is other cars? in my lane or left lane or right lane?
                        bool in_current_lane = (d < (2 + 4 * lane + 2)) && (d > (2 + 4 * lane - 2));
                        bool in_left_lane = (d < (2 + 4 * (lane - 1) + 2)) && (d > (2 + 4 * (lane - 1) - 2));
                        bool in_right_lane = (d < (2 + 4 * (lane + 1) + 2)) && (d > (2 + 4 * (lane + 1) - 2));
                        bool is_close = abs(check_car_s - car_s) < 20; // 30
                        bool in_front = check_car_s > car_s;

                        // a car and mine are in the same lane and very close
                        if (in_current_lane && is_close)
                        {
                            if (in_front)
                            {
                                slow_down = true;
                        ...
                        // check the left lane is safe enough to change lane
                        if (in_left_lane && is_close)
                        {
                            left_lane_empty = false;
                        }
                        // check the right lane is safe enough to change lane
                        if (in_right_lane && is_close)
                        {
                            right_lane_empty = false;
                        }
                    }

                    // Decelerate to avoid a collision and change lane if desired lane is empty
                    if (slow_down)
                    {
                        ref_vel -= ms2mph(.1); //.2236936 ;
                        cout << "slow down!" << endl;
                    ...
                    // Accelerate when it is avaiable
                    else if (ref_vel < 49.5)
                    {
                        ref_vel += ms2mph(.1); //.2236936;
                        if (keep_fast)
                        {
                            ref_vel += ms2mph(0.05); //.2236936 / 2;
                            cout << "keep fast!" << endl;
                        }
                    }
                    ...
                    for (int i = 0; i < ptsx.size(); i++ ) {
                        // Make the (ref_x, ref_y) to the center of the coordinate
                    ...
                    for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
                        //
                        double N = (target_dist / (.02 * ref_vel * 0.44704));
                        ...
                        // cout << "x_piont:" << x_point << "y_point:" << y_point << endl;
                    }
